...
class OdashAPI(http.Controller):
    ...
    @http.route(['/api/get/models'], type='http', auth='public', csrf=False, methods=['GET'], cors="*")
    ...
    @http.route(['/api/get/dashboard'], type='http', auth='public', csrf=False, methods=['POST'], cors="*")
    def get_visualization_data(self, **kw):
        ...
        try:
            ...
            for config in multiple_configs:
                ...
                metadata = self._prepare_visualization_metadata(model_name, config_id)
                ...
                if visualization_type == 'graph':
                    ...
                    visualization_data = {
                        metadata['response_key']: {
                            ...
                            "configHash": f"-{hashlib.md5(json.dumps(config, sort_keys=True, cls=OdashboardJSONEncoder).encode()).hexdigest()[:8]}"
                        }
                    }
    ...
    @http.route(['/api/get/model_fields/<string:model_name>'], type='http', auth='public', csrf=False, methods=['GET'], cors="*")
    ...
    def _prepare_visualization_metadata(self, model_name, config_id):
        """
        Prepare common metadata for visualization responses.
        
        :param model_name: Name of the model
        :param config_id: Configuration ID
        :return: Dictionary with response key, expiration timestamp, and config hash
        """
        # Generate a response key
        response_key = f"{model_name}-{config_id}"
        
        # Create an expiration timestamp (30 minutes from now)
        expires_at = int(time.time() * 1000) + (30 * 60 * 1000)  # 30 minutes in milliseconds
